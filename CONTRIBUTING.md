...
Thank you for your interest in contributing to variantCell! This
document outlines how you can contribute to this R package for
single-cell SNP analysis.
...
Conduct (we follow the [Contributor
Covenant](https://www.contributor-covenant.org/)). Please ensure all
interactions are respectful, inclusive, and professional.
...
- Core dependencies: R6, data.table, Matrix, ggplot2, cowplot,
  GenomicRanges, IRanges, AnnotationHub, matrixStats, ensembldb,
  circlize, ComplexHeatmap, parallel, doParallel, foreach
- Optional: Seurat (for Seurat objects), SingleCellExperiment (for SCE
  objects)
...
2.  Make your changes, following the style guidelines below Test your
    changes with example data (see Testing)
...
1.  Update the README.md and documentation with details of changes if
    appropriate
2.  Update the version number in relevant files following semantic
    versioning
...
We follow the tidyverse style guide for R code with the following
specifics:

1.  Use camelCase for function names within R6 classes, following the
    existing pattern
2.  Add comprehensive Roxygen documentation for all public functions
3.  Include examples in the documentation
4.  For R6 class methods, follow the documentation format shown in the
    existing code
