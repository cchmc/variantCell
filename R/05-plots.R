          ...
          #' @param include_rs_ids Logical. Applies if data_out is set to true
          #' @return
          ...
          variantCell$set("public", "plotSNPs", function(gene,
            ...
            require(ensembldb)           # For SymbolFilter
            ...
            cat(sprintf("\nRetrieving gene coordinates for %s...", gene))
            
            # Use EnsDb directly 
            edb <- EnsDb.Hsapiens.v86
            
            # Get gene and exon information - rest stays the same
            gene_info <- genes(edb, filter = SymbolFilter(gene))
            exon_info <- exons(edb, filter = SymbolFilter(gene))
            
            # Validate gene info
            if(length(gene_info) == 0) {
              stop(sprintf("Gene '%s' not found in EnsDb.Hsapiens.v86. Try checking gene symbol or using a different annotation version.", gene))
            }
            ...
            for(group_idx in seq_along(unique_groups)) {
              ...
              if(!is.null(split.by)) {
                ...
                if(length(base_group_cells) >= params$min_cells) {
                  ...
                  dp_data <- if(use_normalized) {
                    self$snp_database$dp_matrix_normalized[gene_snps, base_group_cells, drop=FALSE]  # FIXED
                  } else {
                    self$snp_database$dp_matrix[gene_snps, base_group_cells, drop=FALSE]  # FIXED
                  }
            ...
            if(data_out) {
              ...
              if(has_rs_ids && include_rs_ids) {
                output_df$rs_id <- self$snp_database$snp_info$rs_id[plot_df$snp_idx]
                
                # Print information about rs# coverage
                rs_count <- sum(!is.na(output_df$rs_id))
                total_snps <- nrow(output_df)
                cat(sprintf("\nrs# identifiers included in output"))
...
#' @param use_rs_ids Logical. Whether to use rs# identifiers for row labels when available.
#'   Default is TRUE. Falls back to chromosome:position format if rs# not available.
#' @param rs_id_format Character. Format for rs# display: "rs_only" (just rs#), "rs_with_pos" 
#'   (rs# with position), or "mixed" (rs# when available, otherwise position). Default is "mixed".
#'
