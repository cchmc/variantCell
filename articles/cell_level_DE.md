...
Cell-level differential SNP analysis focuses on identifying differential
SNP expression between cell clusters.
...
`ident.1` is mandatory, while `ident.2` can optionally be used to
compare to a second identity. Use the `n_cores` parameter to set how
many processing cores will be used.

A dataframe will be returned with the DE SNP results.

## Visualizing DE SNP Expression in Cell Clusters

A heatmap can be generated illustrating DE SNP expression (requires
